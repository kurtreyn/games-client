...
export class GameLobbyApi {
    private _socket: WebSocket | null = null;
    private _isConnected$ = new BehaviorSubject<boolean>(false);

    // A Subject to multi-cast incoming messages to the component
    private _transimissions$ = new Subject<ISocketMessage>();
    ...
    private _buildUrl(): string {
        ...
        return `${baseUrl}${environment.games_endpoint}`;
    }
        ...
        return new Observable<boolean>(observer => {
            ...
            this._socket.onmessage = (event: MessageEvent) => {
                try {
                    ...
                    this._transimissions$.next(incomingMessage);
                } catch (error) {
     ...
     */
    public getTransmissions(): Observable<ISocketMessage> {
        return this._transimissions$.asObservable();
    }

    /**
     * Handles sending data to the open socket
     */
    // public sendTransmission(message: ISocketMessage): boolean {
